...
pub fn extract_filename(data: &Field) -> Result<String, (StatusCode, String)> {
    check_filename(
        ...
        .map(|s| s.to_string())
        .ok_or((StatusCode::BAD_REQUEST, "Missing or Empty Filename".to_string()))?
    )
}

pub async fn save_file(mut data: Field<'_>, filepath: &PathBuf) -> Result<(), (StatusCode, String)>{
    // Create File
    ...
    // Save data to file
    while let Some(chunk) = data.chunk()
        .await
        .map_err(|err| (StatusCode::INTERNAL_SERVER_ERROR, err.to_string()))? {
